#include "dominion.h"
#include <stdio.h>
#include <stdlib.h>


int myAssert(int test, char* msg) {
  if (!test) {
    printf ("Aseert failed: %s\n", msg);
    if (0) {
      exit(1);
    }
  }
  return 0;
}

int main() {

  printf("Feast card test result:\n");

  struct gameState G;
  int k[9] = {smithy,village,  feast, outpost, adventurer, baron,tribute, cutpurse, embargo};

  initializeGame(2, k, 9, &G);

  G.whoseTurn = 1;
  G.hand[1][0] = feast;
  int handCount = G.handCount[1];
  int numActions = G.numActions;
  int deckCount = G.deckCount[1];
  int discardCount = G.discardCount[1];
  int playedCount = G.playedCardCount;
  playCard(0, 0, 0, 0, &G);

  myAssert(G.handCount[1] == handCount, "Feast adds wrong number card for player");
  myAssert(G.deckCount[1] == deckCount, "Feast changes wrong number card on deck");
  
  printf("All tests for Feast passed \n");

}